					...
					<div class="panel-default">
						...
						<div class="panel-body">
							<div class="alert alert-info">
								Veuillez éditer vos informations ci-dessous svp
							</div>
                            @foreach ($users as $user)
                                <form method="POST" action="{{ route('details.update')}}">
                                    @csrf
                                    {{ method_field('PATCH') }}
                                    <input type="text" class="form-control1 control3" name="nom" placeholder="Nom" value="{{ $user['nom']}}" required>
                                    <input type="text" class="form-control1 control3" name="prenom" value="{{ $user['prenom']}}" placeholder="Prénom" required>
                                    <input type="email" class="form-control1 control3" name="email" value="{{ $user['email']}}" placeholder="Email" required>
                                    <input type="text" class="form-control1 control3" name="adresse" value="{{ $user['adresse']}}" placeholder="Adresse" required>
                                    <input type="tel" class="form-control1 control3" name="tel" placeholder="Téléphone" value="{{ $user['tel']}}" required>
